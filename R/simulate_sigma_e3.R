...
run_sigma_e3_simulation <- function(nsamp = 10000, nsim = 2500, seed = 123,
                                    sigma2_e3_values = sigma_e3_grid(),
                                    alpha_z = 0.5,
                                    params = default_parameters()) {
  stopifnot(length(sigma2_e3_values) > 0)

  set.seed(seed)
  nl <- length(sigma2_e3_values)
  ...
  for (i in seq_len(nsim)) {
    ...
    for (j in seq_along(sigma2_e3_values)) {
      sigma2_e3 <- sigma2_e3_values[j]
      e3 <- rnorm(nsamp, 0, sqrt(sigma2_e3))
      ...
      sigma2_d <- var(d)
      sigma2_e3_sample <- var(e3)
      denom_crude <- sigma2_d - (params$gamma_x * alpha_z + params$alpha_x)^2 * sigma2_x
      denom_adjusted <- denom_crude - alpha_z^2 * sigma2_e3_sample
  ...
  summarise_bias_results(
    scenario_values = sigma2_e3_values,
